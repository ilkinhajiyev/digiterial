...
export default function ServicesList({ label, heading }: { label: string; heading: string }) {
  ...
  return (
    <section id="services" className="scroll-mt-24 py-20 md:py-32 bg-paper text-ink"><div className="wrap">
      ...
      <div className="mt-10 border border-ink/15 overflow-hidden rounded-[1.75rem] shadow-[0_24px_70px_-40px_rgba(8,10,9,.35)]">
        {services.map((s, idx) => (
          <Link key={s.slug} href={`/xidmetler/${s.slug}`}
            className="group grid grid-cols-[40px_1fr_auto] sm:grid-cols-[64px_1fr_auto] gap-4 sm:gap-6 items-center px-4 sm:px-7 py-5 sm:py-7 bg-white hover:bg-ink border-b last:border-b-0 border-ink/10 transition-all duration-300 hover:pl-6 sm:hover:pl-9">
