...
export default function SiteHeader({ logoUrl, brand = 'Digiterial' }: { logoUrl?: string; brand?: string }) {
  ...
  return (
    <>
      <header className={`fixed top-0 inset-x-0 z-50 border-b transition-all duration-300 ${scrolled || open ? 'bg-[#080a09]/95 backdrop-blur-md border-white/10' : 'bg-[#080a09]/70 border-transparent'}`}>
        <div className="wrap flex items-center justify-between h-[72px] md:h-[78px]">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            <nav className="hidden lg:flex items-center gap-1 rounded-full border border-white/10 bg-white/[.035] px-1.5 py-1">
              {items.map((i) => (
                <Link key={i.key} href={i.href} className="text-[.8rem] text-neutral-300 px-3 py-2 hover:text-brand transition">
                  {t(i.key)}
                </Link>
              ))}
            </nav>
            <Link href="/elaqe" className="hidden lg:inline-flex hbtn hbtn-y text-sm py-2.5">{t('cta')}</Link>
            <button aria-label="Menyu" aria-expanded={open} aria-controls="mobile-navigation" onClick={() => setOpen(!open)}
      ...
      <div id="mobile-navigation" aria-hidden={!open} inert={!open ? true : undefined} className={`lg:hidden fixed inset-0 z-40 bg-ink transition-all duration-400 ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
