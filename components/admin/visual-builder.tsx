...
export default function VisualBuilder({ initial, settings }: { initial: { seo_title: string; slug: string; meta_desc: string; blocks: any[] }; settings: any }) {
  ...
  async function saveSet() { if(pending) return; setPending(true); try { const r = await saveSettings(set); setSmsg(r.ok ? '✓' : '⚠'); setTimeout(() => setSmsg(''), 2000); } finally { setPending(false); } }
  ...
  async function save() { if(pending) return; setPending(true); try { const clean = blocks.map(({ _id, ...b }) => b); const r = await savePage('home', 'az', clean, seo); setMsg(r.ok ? '✓ Saytda yeniləndi' : '⚠ ' + (r.error || 'Xəta')); setTimeout(() => setMsg(''), 2500); } finally { setPending(false); } }
