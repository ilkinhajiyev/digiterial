  ...
  return (
    <>
      ...
      <section className="pt-32 md:pt-40 pb-12 border-b border-white/15"><div className="wrap">
        <Link href="/isler" className="font-mono text-xs text-mut hover:text-brand transition">← Bütün işlər</Link>
        ...
        {it.url && it.url !== '#' && <a href={it.url} target="_blank" className="inline-flex hbtn hbtn-y mt-7">Saytı ziyarət et ↗</a>}
      </div></section>

      {it.body && (
        <section className="py-16 md:py-20 border-b border-white/15"><div className="wrap grid md:grid-cols-[.35fr_.65fr] gap-8">
          <div className="elbl h-fit">Layihə haqqında</div>
      ...
      {gallery.length > 0 && (
        <section className="py-16 md:py-20"><div className="wrap">
          <div className="elbl mb-8">Qalereya</div>
          <Gallery images={gallery} />
        </div></section>
      )}

      <section className="bg-brand text-ink py-16 md:py-20"><div className="wrap">
        <h2 className="font-display font-bold text-[clamp(2rem,6vw,4rem)] max-w-[14ch] leading-[.95]">Bənzər nəticə istəyirsiniz?</h2>
        <div className="flex justify-between items-end gap-6 flex-wrap mt-8">
          <p className="max-w-[40ch] text-lg">Pulsuz audit və konsultasiya ilə başlayaq.</p>
          <Link href="/elaqe" className="hbtn hbtn-d">Layihə başlat ↗</Link>
        </div>
      </div></section>
    </>
  );
